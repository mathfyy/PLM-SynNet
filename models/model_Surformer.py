...
class MIL_Sum_FC(nn.Module):
    def __init__(self, size_arg="small", dropout=0.25, n_classes=4):
        ...
        """
        super(MIL_Sum_FC, self).__init__()
...
class MIL_Attention_FC_Surformer(nn.Module):
    def __init__(self, size_arg="small", dropout=0.25, n_classes=2, wsi_size=512):
        super(MIL_Attention_FC_Surformer, self).__init__()
        ...
        self.non_lin = nn.Sequential(nn.Linear(wsi_size, 512),
                                     nn.LayerNorm(512),
    ...
    def forward(self, x_path):
        ...
        if self.training:
            logits_gl, hazards_gl, Y_hat_gl = self.forward_global(x_global)
            logits_lo, hazards_lo, Y_hat_lo = self.forward_local(x_local)
            logits_ov, hazards_ov, Y_hat_ov = self.forward_overall(torch.cat((x_global, x_local), dim=-1))

            return logits_gl, hazards_gl, Y_hat_gl, logits_lo, hazards_lo, Y_hat_lo, logits_ov, hazards_ov, Y_hat_ov
        else:
            logits_ov, hazards_ov, Y_hat_ov = self.forward_overall(torch.cat((x_global, x_local), dim=-1))
            return logits_ov, hazards_ov, Y_hat_ov
