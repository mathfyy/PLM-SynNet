...
import models.foundation_model.titan.modeling_titan
...
def get_encoder(model_name, target_img_size=224):
    ...
    if model_name == 'resnet50_trunc':
        ...
        import json
        local_dir = '/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/pretrain_model/Virchow_model/'
        ...
        model.load_state_dict(torch.load(join(local_dir, "pytorch_model.bin"), map_location="cpu"), strict=True)
    elif model_name == 'Virchow2':
        from os.path import join
        import json
        local_dir = '/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/pretrain_model/Virchow2_model/'
        config = json.load(open(join(local_dir, "config.json")))
        model = timm.create_model(
            model_name=config['architecture'],
            checkpoint_path=join(local_dir, "pytorch_model.bin"),
        ...
        # 模型名和参数对应不上，作者未公开对应关系。
        from os.path import join
        import json
        local_dir = '/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/pretrain_model/H-optimus-0_model/'
        config = json.load(open(join(local_dir, "config.json")))
        model = timm.create_model(
        ...
        curPath = '/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/pretrain_model/TITAN_model/'
        # cachePath = '/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/pretrain_model/TITAN_model/'
        # titan = AutoModel.from_pretrained(curPath, local_files_only=True, trust_remote_code=True)
        # model, _ = titan.return_conch()

        import json
        from os.path import join
        config = json.load(open(join(curPath, "config.json")))
        config1 = models.foundation_model.titan.configuration_titan.TitanConfig(config['conch_config'])
        model, _ = models.foundation_model.titan.modeling_titan.Titan.return_conch(config1)
