...
from .conch_tokenizer import Conch_Tokenizer

from .configuration_titan import TitanTextConfig
...
class TextTransformer(nn.Module):
    ...
    def build_cls_mask(self, text, cast_dtype: torch.dtype):
        ...
        cls_mask = F.pad(cls_mask, (0, 1, cls_mask.shape[2], 0), value=True)
    ...
    def forward(self, input_ids):
        ...
        if self.cls_emb is not None:
            ...
            cls_mask = self.build_cls_mask(input_ids, cast_dtype)
            if attn_mask is not None:
        ...
        if self.cls_emb is not None:
            ...
            x = self.ln_final(x)
            pooled, tokens = text_global_pool(x, input_ids, pool_type=self.pool_type)
