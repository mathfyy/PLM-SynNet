...
from models.foundation_model.builder2 import get_encoder
...
def compute_w_loader(model_name, output_path, loader, model, verbose = 0, is_svd = False):
	"""
	args:
	...
	for count, data in enumerate(tqdm(loader)):
		with torch.inference_mode():	
			...
			if model_name == 'conch_v1_5':
				batch = torch.nn.functional.interpolate(batch, scale_factor=2, mode='bilinear', align_corners=False)
			output = model(batch)
			if model_name == 'Virchow':
				class_token = output[:, 0]
				patch_tokens = output[:, 1:]
				embedding = torch.cat([class_token, patch_tokens.mean(1)], dim=-1)
				features = embedding.cpu().numpy().astype(np.float32)
				...
				features = output.cpu().numpy().astype(np.float32)
...
parser.add_argument('--feat_dir', type=str, default='/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/PCA-EPE/patch_feature_Virchow/')
parser.add_argument('--is_svd', type=str, default= False)
parser.add_argument('--slide_ext', type=str, default= '.svs')
parser.add_argument('--model_name', type=str, default='Virchow', choices=['resnet50_trunc', 'uni_v1', 'Virchow', 'Virchow2', 'H-optimus-0', 'prov-gigapath', 'uni2-h', 'conch_v1', 'conch_v1_5', 'CHIEF-Ctranspath', 'MUSK'])
# resnet50-1024
# uni-1024
# Virchow-2560
# Virchow2-2560
...
if __name__ == '__main__':
	...
	for bag_candidate_idx in tqdm(range(total)):
		...
		output_file_path = compute_w_loader(args.model_name, output_path, loader = loader, model = model, verbose = 1, is_svd = args.is_svd)
...
# nohup python -u /data1/fengyy/Code/LRENet_debug/dataProcess/extract_features_fp.py >/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/PCA-EPE/feature_extract_.log 2>&1 &
# nohup python -u /data1/fengyy/Code/LRENet_debug/dataProcess/extract_features_fp1.py >/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/PCA-EPE/feature_extract1_.log 2>&1 &
# nohup python -u /data1/fengyy/Code/LRENet_debug/dataProcess/extract_features_fp3.py >/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/PCA-EPE/feature_extract3_.log 2>&1 &

# nohup python -u /data1/fengyy/Code/LRENet_debug/dataProcess/extract_features_fp2.py >/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/PCA-EPE/feature_extract2_.log 2>&1 &
# nohup python -u /data1/fengyy/Code/LRENet_debug/dataProcess/extract_features_fp4.py >/data1/dataset/gbm_data/pipeline_data_usecaptk/fengyy/BrainTumorPath/PCA-EPE/feature_extract4_.log 2>&1 &